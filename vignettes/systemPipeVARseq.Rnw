...
%\VignetteDepends{rjson, ggplot2, limma, edgeR, GOstats, GO.db, annotate, pheatmap, VariantAnnotation, VariantTools}
...
The following performs variant calling with \Robject{GATK} and \Robject{BCFtools} in parallel mode on a compute cluster \citep{McKenna2010-ql, Li2011-ll}. If a cluster is not available, the \Rfunction{runCommandline()} function can be used to run them sequentially on a single machine. Typically, the user would choose here one variant caller rather than running several ones.   
...
The function \Rfunction{filterVars} filters VCF files based on user definable quality parameters. It first imports each VCF file into R, applies the filtering on an internally generated \Robject{VRanges} object and then writes the results to a new VCF file.
