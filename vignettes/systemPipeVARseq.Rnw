...
The following runs the variant calling with BCFtools. This step requires in the current working directory the parameter file \Robject{sambcf.param}. A bash script is not required for this step.  
<<eval=FALSE, keep.source=TRUE>>=
