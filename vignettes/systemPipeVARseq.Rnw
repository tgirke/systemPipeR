...
An alternative variant tolerant aligner is \Rfunction{gsnap} from the \Rpackage{gmapR} package. The following code shows how to run this aligner on multiple nodes of a comute cluster that uses Torque as scheduler.
<<eval=FALSE>>=
