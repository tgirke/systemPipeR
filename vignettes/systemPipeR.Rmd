...
(BED/WIG files). Each sample level input/outfile operation uses its own _`SYSargs`_ 
...
operations with any combinaton of command-line or R-based software. 
...
sequence (FASTA) and its GFF annotion files (provided in the same download) have 
...
necessary update the corresponding _`.cwl`_ and _`.yml`_ files. A collection of pre-generated _`.cwl`_ and _`.yml`_
files is provided in the _`param/cwl`_ subdirectory of each workflow template. They
...
targets <- system.file("extdata", "targets.txt", package="systemPipeR")
dir_path <- "param/cwl/trim/trim-se" ## TODO: replace path to system.file 
...
dir_path <- "param/cwl/trim/trim-pe" ## TODO: replace path to system.file 
...
preprocessReads(args=trim, Fct="filterFct(fq, cutoff=20, Nexceptions=0)", 
                batchsize=100000)
writeTargetsout(x=trim, file="targets_PEtrim.txt", step=1)
```

### Preprocessing with Trim Galore

[Trim Galore](http://www.bioinformatics.babraham.ac.uk/projects/trim_galore/) is 
...
targets <- system.file("extdata", "targets.txt", package="systemPipeR")
dir_path <- "param/cwl/trim_galore/trim_galore-se" ## TODO: replace path to system.file 
trimG <- loadWorkflow(targets=targets, wf_file="trim_galore-se.cwl", input_file="trim_galore-se.yml", dir_path=dir_path)
trimG <- renderWF(trimG, inputvars=c(FileName="_FASTQ_PATH_", SampleName="_SampleName_"))
trimG
cmdlist(trimG)[1:2]

## Run Single Machine Option
runCommandline(trimG, make_bam = FALSE)
```
...
Parallelization of FASTQ quality report on single machine with multiple cores.
...
f <- function(x) {
  library(systemPipeR)
  targetsPE <- system.file("extdata", "targetsPE.txt", package="systemPipeR")
  dir_path <- "param/cwl/trim-pe" ## TODO: replace path to system.file 
  trim <- loadWorkflow(targets=targetsPE, wf_file="trim-pe.cwl", input_file="trim-pe.yml", dir_path=dir_path)
  trim <- renderWF(trim, inputvars=c(FileName1="_FASTQ_PATH1_", FileName2="_FASTQ_PATH2_", SampleName="_SampleName_"))
...
dir_path <- "param/cwl/hisat2/hisat2-idx" ## TODO: replace path to system.file 
idx <- loadWorkflow(targets=NULL, wf_file="hisat2-index.cwl", input_file="hisat2-index.yml", dir_path=dir_path)
idx <- renderWF(idx, inputvars=c(FileName="_FASTQ_PATH_", SampleName="_SampleName_"))
idx
cmdlist(idx)
...
step. When this options are used, the output location will be update by default 
...
align <- runCommandline(align, dir=TRUE, make_bam = TRUE) ## same as above but writes files to ./results/workflowName/Samplename folders and converts *.sam files to sorted and indexed BAM files. Assigning the new location of the output files to the object align.
```
...
```{r bwa-pe_alignment24, eval=FALSE}    
targetsPE <- system.file("extdata", "targetsPE.txt", package = "systemPipeR")
...
BWA Workflow PE:
```{r bwa-pe_alignment2, eval=FALSE}    
targetsPE <- system.file("extdata", "targetsPE.txt", package = "systemPipeR")
...
BWA Workflow SE
```{r bwa-pe_alignment21, eval=FALSE}    
targetsSE <- system.file("extdata", "targets.txt", package = "systemPipeR")
