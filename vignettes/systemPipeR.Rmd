...
runCommandline(align[1], make_bam = FALSE) ## generates alignments and writes *.sam files to ./results folder 
...
on a cluster allow to run all 18 processes at the same time then the shown sample 
...
output(bwaPE)[1:2]
## Single Machine
runCommandline(args= bwaPE, make_bam=FALSE) 
...
## Single Machine
runCommandline(args=rsubread[1]) 
```
...
WF_track <- run_track(WF_ls = c(align, WF, tophat2PE, bowtiePE, bwa.pe, starPE, rsubread, gsnap))
names(WF_track)
WF_steps(WF_track)
track(WF_track)
summaryWF(WF_track)
