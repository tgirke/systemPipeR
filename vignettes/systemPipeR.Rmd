...
sal <- SPRproject(restart = TRUE, logs.dir = ".SPRproject", 
...
with `load.envir` argument. Please note that the environment was saved only with you run the workflow 
...
- Copy one or multiple objects from the running environment to a new enviroment:

```{r}
copyEnvir(sal, list = c("df"), new.env = globalenv(), silent = FALSE)
```
...
replaceCodeLine(sal_c, step="export_iris", line = 2) <- "log_cal_100 <- log(50)"
...
renameStep(sal_c, c(1, 2)) <- c("newStep", "newIndex")
sal_c
