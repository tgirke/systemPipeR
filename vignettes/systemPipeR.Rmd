...
### Workflow template from Package
...
This tutorial introduces the basic ideas and tools needed to build a specific workflow from scratch or modified an existing workflow.
...
If you desire to build your pipeline with any different software then Hisat2, make sure to have the respective software installed and configured in your PATH. To make sure if the configuration is right, you always can test as follow:

```{r test_software, eval=FALSE}
tryCL(command="hisat1") 
tryCL(command="hisat2") 
```

## Project Initialization 

The Project management structure is essential, especially for reproducibility and efficiency in the analysis. Here we show how to create a simple and complete R object list storing a all the information about the project, as the path of the data and results, as well all the setting for the particular project and the Initial annotation file (targets), storing all the initial files for the project and all the metadata data related with the project. The list can be saved as a simple YAML file. 
Note the workflow steps and all the analysis is defined in the RMarkdown file. 
