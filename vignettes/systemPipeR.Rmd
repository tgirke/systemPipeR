...
analysis step (see Figure \@ref(fig:sysargslist_image)).
...
```{r sysargslist_image, eval=TRUE, echo=FALSE, out.width="100%", fig.align = "center", fig.cap= "Workflow steps with input/output file operations are controlled by `SYSargs2` objects. Each `SYSargs2`instance is constructed from one targets and two param files. The only input provided by the user is the initial targets file. Subsequent targets instances are created automatically, from the previous output files. Any number of predefined or custom workflow steps are supported. One or many `SYSargs2` objects are organized in an `SYSargsList` container."}
...
targets files are created automatically (see Figure \@ref(fig:targets_fig)).
...
```{r targets_fig, eval=TRUE, echo=FALSE, out.width="100%", fig.align = "center", fig.cap= "_`systemPipeR`_ automatically creates the downstream `targets` files based on the previous steps outfiles. A) Usually, users provide the initial `targets` files, and this step will generate some outfiles, as demonstrated on B. Then, those files are used to build the new `targets` files as inputs in the next step. _`systemPipeR`_ (C) manages this connectivity among the steps automatically for the users."}
...
In the [next section](#sysargslist), we will demonstrate how to build the workflow in an
...
## Build workflow interactive {#sysargslist}
...
the `input` variables defined in the `*.yml` files (see Figure \@ref(fig:spr_cwl)). 
...
The final step in this simple workflow is a R code step. For that, we are using the 
`LineWise` constructor function as demonstrated above. 

One interesting feature here is the `getColumn` method that allows extract the information 
...
appendStep(sal) <- LineWise(code = {
                    ...
                    ggplot2::ggplot(stats, ggplot2::aes(x=species, y=mean, fill=species)) + 
...
SPR workflows can be visualized with the `plotWF` function.
This function will make a plot of selected workflow instance and following 
information is displayed on the plot: 
    - Workflow structure (dependency graphs between different steps); 
    - Workflow step status, i.e. success, fail, pending; 
    - Sample status and statistics; 
    - Workflow timing: start, end and duration time. 

If no argument is provided, the basic plot will automatically detect width, height, layout, plot method, branches, _etc_. 

```{r, eval=TRUE}
plotWF(sal, show_legend = TRUE, width = "80%", height="250px", rstudio = TRUE)
```

For more details, please see additional information here.
...
For complete information, please check the full documentation [here](https://www.commonwl.org/v1.2/CommandLineTool.html) 
for CommandLineTools and [here](https://www.commonwl.org/v1.2/Workflow.html) for Workflows and the user guide [here](https://www.commonwl.org/user_guide/). 
...
```{r spr_cwl, eval=TRUE, echo=FALSE, out.width="100%", fig.align = "center", fig.cap= "WConnectivity between CWL param files and targets files."}
...
# Visualize Workflow - Full details 
