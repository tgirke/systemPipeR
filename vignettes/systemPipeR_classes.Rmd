...
appendStep(sal) <- LineWise(stepName = "R_read", 
                       code = "hello <- lapply(getColumn(sal, step=1, 'outfiles'), function(x) yaml::read_yaml(x))", dependency = "")
```
...
```{r, eval=TRUE}
plotWF(sal, show_legend = FALSE, width = "80%", height="250px")
```
...
```{r}
appendStep(sal) <- LineWise("log_cal <- log(1)", "R_code")
sal
stepsWF(sal)
```
...
```{r, eval=TRUE}
sal_sub <- subsetTargets(sal, subset_steps = c(1,3), input_targets = 1:4, keep_steps = TRUE)
...
```{r}
sal_sub <- subsetTargets(sal, subset_steps = c(1,3), input_targets = ("M1A"), keep_steps = TRUE)
stepsWF(sal_sub)
targetsWF(sal_sub)
outfiles(sal_sub)
```
