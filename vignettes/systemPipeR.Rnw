...
The \Robject{targets} file defines all FASTQ files and sample comparisons of an analysis workflow. The following shows the format of a sample \Robject{targets} file provided by this package. In target files for single end (SE) data sets, the first three columns are mandatory including their column names, while it is four mandatory columns for PE data. All subsequent columns are optional and any number of additional columns can be added as needed.
...
\noindent Sample comparisons are defined in the header lines of the \Robject{targets} file starting with '\texttt{\# <CMP>}'. The function \Rfunction{readComp} imports the comparison and stores them in a \Robject{list}. These lines are also optional. They are mainly useful for controlling comparative analysis according to certain biological expectations, such as simple pairwise comparisons in RNA-Seq experiments. 
<<eval=TRUE, keep.source=TRUE>>=
...
<<sample_tree, eval=TRUE, fig=TRUE, include=FALSE, keep.source=TRUE>>=
library(ape)
...
\begin{figure}[H]
  \centering
   \includegraphics[width=6cm]{systemPipeR-sample_tree.pdf}
...
<<DEGcounts, eval=TRUE, fig=TRUE, include=FALSE, keep.source=TRUE>>=
DEG_list <- filterDEGs(degDF=edgeDF, filter=c(Fold=2, FDR=10))
@
\begin{figure}[H]
  \centering
   \includegraphics[width=10cm]{systemPipeR-DEGcounts.pdf}
...
<<vennplot, eval=TRUE, fig=TRUE, include=FALSE, keep.source=TRUE>>=
...
\begin{figure}[H]
  \centering
   \includegraphics[width=14cm]{systemPipeR-vennplot.pdf}
