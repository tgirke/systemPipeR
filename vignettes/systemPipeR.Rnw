...
The \Robject{targets} file defines all FASTQ files and sample comparisons of an analysis workflow. The following shows the format of a sample \Robject{targets} file provided by this package. Note: the first three columns are mandatory including their column names. All subsequent columns are optional and any number of additional columns can be added as needed.
...
\noindent Beginning of the sample alignment stats file provided by this package. Note, for simplicity the number of reads is multiplied here by 2 to approximate proper alignment frequencies for PE samples. 
<<eval=TRUE, keep.source=TRUE>>=
