...
\title{Analysis Workflow for Ribo-Seq and polyRibo-Seq Experiments}
...
experiments that utilize RNA subpopulations directly bound to ribosomes. Their
readout of gene expression provides a better approximation of downstream
protein abundance profiles than standard RNA-Seq because the samples are
enriched for RNA populations tightly linked with translational processes.
polyRibo-Seq utilizes polyribosomal RNA isolates for sequencing, whereas
Ribo-Seq is a footprinting approach restricted to sequencing RNA fragments
protected by ribosomes \citep{Ingolia2009-cb}. 
...
The most important analysis steps of this workflow include read preprocessing,
read alignments against a reference genome, read counting for a wide range of
genomic features (\textit{e.g.} CDSs, UTRs, uORFs, rRNAs, etc.), differential
gene expression and differential ribosome binding analyses, as well as a
variety of genome-wide summary plots for visualizing RNA expression trends.
Functions are provided that enable evaluation of the quality of Ribo-seq data,
discovery of novel expressed regions in the genomes and provide insights into
gene regulation at the post-transcriptional and translational levels. For
instance, the functions \Rfunction{genFeatures} and
\Rfunction{featuretypeCounts} can be used to quantify  the expression output
for all feature available in a genome annotation (\textit{e.g.} genes, introns,
exons, miRNAs, intergenic regions, etc.). To determine the approximate read
length of ribosome footprints in Ribo-Seq experiments, the feature type counts
can be obtained and plotted for specific read lengths separately.  Typically,
the most abundant read length obtained for translated features corresponds to
the true footprint length occupied by the ribosomes for a given organism group.
Based on the results from several Ribo-Seq studies, ribosome footprint are
typically ~30 nucleotides long. However,  their length can vary by several
nucleotides depending upon the optimization of the RNAse digestion and probably
also factors associated with translational regulation. For Ribo-Seq experiments
it is also useful to monitor the abundance of reads mapping to rRNA genes due
to the high rRNA content of ribosomes. 
...
of the phasing of ribosome movements along the triplets of coding sequences.
...
Ribo-seq data can also be used to evaluate aspects of translational control due
to the ribosome occupancy in upstream open reading frames (uORFs). The latter
are frequently present in (or near) the 5' UTRs of transcripts. The function
\Rfunction{predORFs} identifies ORFs in the nucleotide sequences of transcripts or
their subcomponents such as UTR regions. After scaling the resulting ORF
coordinates back to the genome with \Rfunction{scaleRanges}, one can use these
novel features (\textit{e.g.} uORFs) for the same expression analysis routines
as for pre-existing annotations. For instance, in Ribo-Seq experiments one can
systematically identify all transcripts containing ribosomes in their uORF
regions. The binding of ribosomes in uORF regions may indicate a regulatory role 
in the translation of the downstream main ORF and/or the translation of the 
uORFs into functionally relevant peptides. 
...
user to prove any custom set of triplets under the \Rfunarg{startcodon} and \Rfunarg{stopcodon}
arguments. The argument \Rfunarg{n} defines the maximum number of ORFs to return for each
input sequence (\textit{e.g.} \Rfunarg{n=1} returns only the longest ORF). With
\Rfunarg{n="all"} and \Rfunarg{longest\_disjoint=TRUE}, the function will
return all non-overlapping ORFs including the longest ORF for each sequence.
...
(corresponding to stop codon). The following example does this for a compound
uORF containing three exons.
<<eval=FALSE>>=
translate(unlist(getSeq(FaFile(genome), grl_scaled[[7]])))
@

\subsection{Adding custom features to other feature types}
If required the any custom feature ranges can be added to the standard features
...
\section{Differential ribosome binding analysis (tranlational efficiency)}
To be added...
