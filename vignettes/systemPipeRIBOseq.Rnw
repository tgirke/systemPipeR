...
among the two is that polyRibo-Seq utilizes polyribosomal RNA isolates for sequencing,
...
For test purposes we generate here from the sample data used in this vignette a
small test data set, where we include two types of RNA isolates
(\Rfunarg{assays}): polyribosomal mRNA (\Rfunarg{Ribo}) and total mRNA
(\Rfunarg{mRNA}). In addition, there are two treatments
...
for the two conditions: \\ $(Ribo\_A1 / mRNA\_A1) / (Ribo\_M1 / mRNA\_M1)$. The latter
can be modeled with the \Rpackage{DESeq2} package using the design $~ assay +
...
in condition \Rfunarg{A1} than in \Rfunarg{M1} as follows.
