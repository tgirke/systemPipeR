...
\section{Differential ribosome binding analysis (translational efficiency)}

For test purposes the following generates a small test data set from the sample
data used in this vignette, where two types of RNA samples (\Rfunarg{assays})
are included: polyribosomal mRNA (\Rfunarg{Ribo}) and total mRNA
(\Rfunarg{mRNA}). In addition, there are two treatments (\Rfunarg{conditions}):
\Rfunarg{M1} and \Rfunarg{A1}. 
...
for the two conditions: \\ $(Ribo\_A1 / mRNA\_A1) / (Ribo\_M1 / mRNA\_M1)$. 
The latter can be modeled with the \Rpackage{DESeq2} package using the design $~ assay +
