...
The following identifies continuous ORFs in intergenic regions. Note, \Rfunction{predORF} can only identify continuous ORFs in a set of query sequences. The function does not identify and remove introns prior to the ORF prediction.
<<eval=FALSE>>=
