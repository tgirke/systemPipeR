...
featureCoverage <- function(bfl, grl, resizereads=NULL, readlengthrange=NULL, Nbins=20, method=mean, fixedmatrix, resizefeatures, upstream, downstream, outfile, overwrite=FALSE) {
    ...
    for(mysample in seq_along(bfl)) {
        ...
        ## Sense coverage
        cov_posreg <- suppressWarnings(Views(cov_pos, as(gr, "RangesList")))
        cov_posreg <- cov_posreg[sapply(cov_posreg, length) > 0] # Removes empty components (chr) 
        ## Antisense coverage
        cov_negreg <- suppressWarnings(Views(cov_neg, as(gr, "RangesList")))
...
plotfeatureCoverage <- function(covMA, method=mean, scales="fixed", scale_count_val=10^6) {
    ...
    mymax <- mymax * 1.3 # add 30% to limit
...
## to consider.
.iterTrimbatch <- function(fq, pattern, minpatternlength=8, Nnumber=1, polyhomo=20, minreadlength=18, maxreadlength) {
    ...
    for(j in seq(along=minpatternlength:(nchar(pattern)))-1) {
        ...
        cat(c(length(fq)), length(processedreads), length(fq) + length(processedreads), "\n")
    }
