...
loadWorkflow <- function(targets=NULL, wf_file, input_file, dir_path=".") {
  ...
  if(tolower(wf$class) == "workflow") { 
    ...
    cltlist <- sapply(cltpaths, function(x) yaml::read_yaml(file.path(dir_path, x))) 
...
# WF <- loadWorkflow(targets=targets, wf_file="hisat2-se/hisat2-mapping-se.cwl", input_file="hisat2-se/hisat2-mapping-se.yml", dir_path=dir_path)
