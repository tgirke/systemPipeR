...
- Helper package to generate with a single command NGS workflow templates for _`systemPipeR`_ environment. 
- Includes sample data for testing.
- Workflows can be changed and extended by user.
...
names(args)[c(5,8,13)]
```
Autogenerates command-line arguments for a given software on first sample, here for _`Tophat2`_.
...
$ make -B
```
- Analysis reports in PDF or HTML format are autogenerated at the end of each workflow using standard R resources for scientific report generation including _`knitr`_ and _`rmarkdown`_, respectively.
