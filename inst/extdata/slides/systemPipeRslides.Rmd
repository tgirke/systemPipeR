...
-   [_systemPipeR_](http://www.bioconductor.org/packages/devel/bioc/html/systemPipeR.html) is an R package for building *end-to-end* analysis pipelines with automated report generation for next generation for NGS applications 
-   Important features:
    - Support for command-line software, such as NGS aligners, peak callers, variant callers, etc.
...
- Each workflow step is controlled by a _`SYSargs`_ object [@Girke2014-oy].

- _`SYSargs`_ instances are constructed from a _`targets`_ file and a _`param`_ file

- Only input provided by user is first _`targets`_ files. Subsequent _`target`_ instances are created automatically. 
...
- Alternatively, one can run an entire workflow from start to finish with a single command by executing from the command-line _`'make -B'`_. 
